...
class VanillaPipeline(Pipeline):
    ...
    def get_average_eval_image_metrics(
        ...
        image_set = self.datamanager.fixed_indices_eval_dataloader
        with Progress(
            ...
            idx = 0
            for camera, batch in image_set:
                with torch.no_grad():
                    torch.cuda.synchronize()
                    inner_start = time()
                    outputs = self.model.get_outputs_for_camera(camera=camera)
                    torch.cuda.synchronize()
                    cur_time = time() - inner_start
                ...
                if output_path is not None:
                    for key, val in images_dict.items():
                        Image.fromarray((val * 255).byte().cpu().numpy()).save(
                            output_path / "{0:06d}-{1}.png".format(int(idx), key)
                        )

                assert "num_rays_per_sec" not in metrics_dict
                metrics_dict["num_rays_per_sec"] = num_rays.item() / cur_time
                fps_str = "fps"
