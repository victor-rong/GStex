...
class Cameras(TensorDataclass):
    ...
    def to_path_json(self) -> Dict:
        path_dict = {
            "render_height": self.height[0].item(),
            "render_width": self.width[0].item(),
            "camera_path": [
            ],
        }
        ...
        for i in range(flattened.shape[0]):
            camera = {
                "camera_to_world": self.camera_to_worlds[i].tolist(),
                "fx": flattened[i].fx.item(),
                "fy": flattened[i].fy.item(),
            }
            path_dict["camera_path"].append(camera)
        return path_dict
...
def json_to_camera(camera_dict: Dict[str, Any]) -> Cameras:
    ...
    c2ws = [torch.tensor(camera_dict["camera_to_world"]).view(3, 4)]
    camera_to_worlds = torch.stack(c2ws, dim=0)

    fx = torch.tensor([camera_dict["fx"]])
    fy = torch.tensor([camera_dict["fy"]])
    cx = torch.tensor([camera_dict["cx"]])
    cy = torch.tensor([camera_dict["cy"]])
